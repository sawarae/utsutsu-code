...
`$AGENT_HOME/hooks/` にTTSスクリプトと設定を配置する。
...
```bash
mkdir -p "$AGENT_HOME/hooks" "$AGENT_HOME/skills"
# フック
ln -sf "$(pwd)/.claude/hooks/mascot_tts.py" "$AGENT_HOME/hooks/mascot_tts.py"
ln -sf "$(pwd)/.claude/hooks/tts_config.toml" "$AGENT_HOME/hooks/tts_config.toml" 2>/dev/null || \
...
mkdir -p "$AGENT_HOME/hooks" "$AGENT_HOME/skills"
# フック
cp .claude/hooks/mascot_tts.py "$AGENT_HOME/hooks/mascot_tts.py"
...
**確認:**
```bash
python3 "$AGENT_HOME/hooks/mascot_tts.py" --emotion Gentle "グローバルフックのテスト"
```
...
| リンク | リンク先 |
|--------|---------|
| `$AGENT_HOME/hooks/mascot_tts.py` | → `.claude/hooks/mascot_tts.py` |
| `$AGENT_HOME/hooks/tts_config.toml` | → `.claude/hooks/tts_config.toml` |
| `$AGENT_HOME/skills/*` | → `.codex/skills/*` または `.claude/skills/*` |
